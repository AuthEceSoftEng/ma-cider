...
export const openModalPdfViewer = pdfLink => {
    document.getElementById('my-pdf-modal').style.display = 'block';
    document.getElementsByTagName('body')[0].style.overflow = 'hidden';
    setTimeout(() => {
        window.PDFObject.embed(pdfLink, "#my-pdf-modal-container");
    }, 150)
}
        ...
        fetch(input + jsonToQuery(query), { ...init, headers })
            ...
            .then(([isJson, data]) => {
                if (isJson) {
                    ...
                    FileSaver.saveAs(data, query.fileName ? `${query.fileName}.pdf` : 'download.pdf');
                    resolve(true)
                }
